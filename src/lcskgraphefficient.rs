...
pub fn anchoring_lcsk_path_for_threading(
    ...
    Vec<usize>,
    Vec<Graph<u8, i32, Directed, usize>>,
    Vec<usize>,
    Vec<Vec<u8>>,
    Vec<Vec<(usize, usize)>>,
) {
    ...
    for (index, pos) in ascending_path.iter().enumerate() {
        ...
        loop {
            ...
            let added_node =
                section_graph.add_node(graph.raw_nodes()[topo_indices[topo_indices_index]].weight);
            ...
            let incoming_nodes: Vec<NodeIndex<usize>> = graph
                .neighbors_directed(NodeIndex::new(topo_indices[topo_indices_index]), Incoming)
                .collect();
            if this_is_head_node == false {
                for incoming_node in incoming_nodes {
                    let mut edges = graph.edges_connecting(
                        incoming_node,
                        NodeIndex::new(topo_indices[topo_indices_index]),
                    );
                    ...
                    section_graph.add_edge(
                        NodeIndex::new(section_incoming_node),
                        added_node,
                        incoming_weight,
                    );
                }
        ...
        if cut_is_made_at_this_point == false {
            ...
            for index in 0..temp_section_lcsk.len() {
                ...
                if temp_section_lcsk[index].0 > query_cut_off_for_lcsk {
                    filtered_section_lcsk.push((
                        temp_section_lcsk[index].0 - query_cut_off_for_lcsk,
                        temp_section_lcsk[index].1,
                    ));
                }
            }
    ...
    for index in 0..temp_section_lcsk.len() {
        ...
        if temp_section_lcsk[index].0 > query_start {
            filtered_section_lcsk.push((
                temp_section_lcsk[index].0 - query_start,
                temp_section_lcsk[index].1,
            ));
        }
    }
...
pub fn try_to_make_the_cut(
    output_graph: &POAGraph,
    topo_index: usize,
    total_num_sequences: usize,
) -> bool {
    ...
    while let Some(neighbour_node) = neighbour_nodes.next() {
        ...
        if number_of_outgoing_node == 1 {
            let mut edges =
                output_graph.edges_connecting(NodeIndex::new(topo_index), neighbour_node);
...
pub fn lcskpp_graph(
    ...
    for (idx, &(x, y)) in kmer_pos_vec.iter().enumerate() {
        ...
        events.push((
            x,
            kmer_graph_index[idx][0],
            (idx + kmer_pos_vec.len()) as u32,
            kmer_path_vec[idx].clone(),
            kmers_previous_node_in_paths[idx].clone(),
            kmer_graph_index[idx].clone(),
        ));
        events.push((
            x + k - 1,
            *kmer_graph_index[idx].last().unwrap(),
            idx as u32,
            kmer_path_vec[idx].clone(),
            kmers_previous_node_in_paths[idx].clone(),
            kmer_graph_index[idx].clone(),
        ...
        if is_start {
            ...
            for path_index in 0..ev.3.len() {
                ...
                if prev_node != u32::MAX {
                    ...
                    let (temp_value, temp_position) =
                        max_bit_tree_path[path].get(prev_node as usize);
...
pub fn better_find_kmer_matches(
    query: &[u8],
    graph_sequences: &Vec<Vec<u8>>,
    graph_ids: &Vec<Vec<usize>>,
    k: usize,
) -> (
    Vec<(u32, u32)>,
    Vec<Vec<usize>>,
    ...
    // aggregated result
    let mut all_result_per_path: Vec<Vec<(u32, u32, u32, u32, Vec<u32>)>> =
        vec![vec![]; graph_sequences.len()]; // seq, graph, graph + k, prev node in path, path index
    ...
    for (index, seq) in graph_sequences.iter().enumerate() {
        ...
        for a_match in matches {
            ...
            all_result_per_path[index].push((
                a_match.0,
                graph_index,
                graph_index_plus_k,
                graph_index_minus_1,
    ...
    let mut loc_to_data: HashMapFx<(u32, u32), (u32, Vec<u32>, Vec<usize>, Vec<u32>)> =
        HashMapFx::default();
...
pub fn find_sequence_in_graph(
    sequence: Vec<u8>,
    graph: &POAGraph,
    topo_indices: &Vec<usize>,
    topo_map: &HashMap<usize, usize>,
    error_index: usize,
) -> (bool, Vec<usize>, Vec<u8>) {
    ...
    if (graph.neighbors(NodeIndex::new(start)).count() == 0) && (temp_sequences.len() > 0) {
        ...
        if temp_sequences.len() > 0 {
            for neighbor in graph.neighbors(NodeIndex::new(start)) {
                dfs_get_sequence_paths(
                    index + 1,
                    temp_sequences.clone(),
                    graph,
                    neighbor.index(),
                    path.clone(),
                    sequence.clone(),
    ...
    for (top_index, section) in graph_sequences.iter().enumerate() {
        for (bottom_index, seq) in section.iter().enumerate() {
            ...
            for a_match in matches {
                ...
                let graph_index_minus_1;
                let graph_index_plus_k =
                    graph_ids[top_index][bottom_index][a_match.1 as usize - 1 + k] as u32;
                if a_match.1 > 0 {
                    graph_index_minus_1 =
                        graph_ids[top_index][bottom_index][a_match.1 as usize - 1] as u32;
                } else {
                    graph_index_minus_1 = u32::MAX;
                }
                // if it is in the result vec, add path if indices match and path different
                if let Some(index_result) = kmers_result_vec
                    .iter()
                    .position(|r| (r.0, r.1, r.2) == (a_match.0, graph_index, top_index))
                {
...
pub fn divide_poa_graph_get_paths(
    output_graph: &POAGraph,
    topo_indices: &Vec<usize>,
    total_num_sequences: usize,
    cut_threshold: usize,
    topo_map: &HashMap<usize, usize>,
) -> (Vec<Vec<Vec<usize>>>, Vec<Vec<Vec<u8>>>, usize) {
    ...
    while topo_indices.len() > current_topo_indices_index + 1 {
        if current_topo_indices_index + 1 < topo_indices.len() {
            if current_topo_indices_index >= current_cut_limit {
                println!("Try cutting...");
                let cut_result = try_to_make_the_cut(
                    output_graph,
                    topo_indices[current_topo_indices_index],
                    total_num_sequences,
                );
            ...
            println!(
                "index {} node {} cut limit {}",
                current_topo_indices_index,
                topo_indices[current_topo_indices_index],
                current_cut_limit
            );
        } else {
    ...
    return (
        all_all_paths,
        all_all_sequences,
        max_number_of_paths_per_section,
    );
}
...
pub fn find_kmer_matches(
    query: &[u8],
    graph_sequences: &Vec<Vec<u8>>,
    graph_ids: &Vec<Vec<usize>>,
    k: usize,
) -> (Vec<(u32, u32)>, Vec<u32>, Vec<Vec<usize>>, Vec<Vec<u32>>) {
    // hash the query
    let set = hash_kmers(query, k);
    // go through the paths and get the indices of path and make a list with query index, graph index, paths
    let mut kmers_result_vec: Vec<(u32, u32)> = vec![];
    ...
    for (index, seq) in graph_sequences.iter().enumerate() {
        ...
        for a_match in matches {
            ...
            if let Some(index_result) = kmers_result_vec
                .iter()
                .position(|r| (r.0, r.1) == (a_match.0, graph_index))
            {
